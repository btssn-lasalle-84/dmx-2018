...
/**
  *\file enttecdmxusb.cpp
  *
  *\brief Definition de la classe EnttecDMXUSB
  *
  *
...
int EnttecDMXUSB::sendPacket(int pkt_type, char *data, int length)
{
    ...
    if(interfaceEnttec == DMX_USB_PRO)
    {
        ...
        ser.write((char *)&len, sizeof(len)); /* 16-bit length of packet LSB-MSB */
        ...
        if(ser.waitForReadyRead(250))
        {
            ...
            else
            {
                ...
                case PKT_DMXIN : /* 5 */
                    {
                        ...
                        if(ser.getChar(&c))
                            dmxin_quality = c; /* next byte is data quality */
                        err = ser.read((char *)&buffer[0],dmxin_length); /* get the dmx data + PKT_EOM */
                ...
                case PKT_DMXIN_UPDATE : /* 9 */ /* recieve a dmx update packet */
                    {
                        dmxin_length =  makeword16(buffer[1],buffer[2]);
                        err=ser.read((char *)&buffer[0],dmxin_length+1);
